...
# 🖥 Service Preview / サービス画面

BookCast의 주요 기능과 실제 구현 화면입니다.
BookCastの主な機能と実際の実装画面です。

---

## 🏠 Main Page / メインページ

![BookCast Main Page](docs/images/main.png)

### 🇰🇷

서비스의 메인 화면입니다.
도서 검색, 추천 도서 및 주요 서비스로 이동할 수 있도록 구성했습니다.

### 🇯🇵

サービスのメイン画面です。
書籍検索、おすすめ書籍、各種サービスへアクセスできるように構成しました.

---

## 🔎 Book Search / 書籍検索

![Book Search](docs/images/search.png)

### 🇰🇷

키워드, 도서명, 저자, ISBN, 출판사 등의 조건을 이용하여 도서를 검색할 수 있습니다.

React에서 입력된 검색 조건을 Spring Boot로 전달하고,
Spring Boot가 외부 공공 도서 API를 호출하여 결과를 가공한 후 프론트엔드에 전달하는 방식으로 구현했습니다.
...
キーワード、書籍名、著者、ISBN、出版社などの条件から書籍を検索できます。

Reactから検索条件をSpring Bootへ送信し、
Spring Bootが外部の公共書籍APIを呼び出してデータを処理した後、フロントエンドへ返す構成にしました。

---

## 📖 Book Detail / 書籍詳細

![Book Detail](docs/images/detail.png)

### 🇰🇷

검색한 도서의 상세 정보를 확인할 수 있는 화면입니다.

도서의 기본 정보뿐만 아니라 도서관 소장 여부와 대출 가능 여부 등의 정보를 함께 확인할 수 있도록 구현했습니다.
...
検索した書籍の詳細情報を確認する画面です。

書籍の基本情報だけでなく、図書館での所蔵状況や貸出可否なども確認できるように実装しました。
...
![Book Request](docs/images/request.png)

### 🇰🇷

도서관에 원하는 도서가 없는 경우 희망도서로 신청할 수 있는 화면입니다.

검색한 도서 정보를 활용해 신청서를 작성하고 Spring Boot API를 통해 신청 데이터를 저장하도록 구현했습니다.
...
図書館に希望する書籍がない場合、その書籍を希望図書として申請できる画面です。

検索した書籍情報を利用して申請を行い、Spring BootのREST APIを通して申請データを保存する構成にしました。
...
로그인한 사용자의 정보와 서비스 이용 내역을 확인할 수 있는 화면입니다.

사용자 인증 정보를 기반으로 개인별 데이터를 조회하도록 구현했습니다.
...
ログインしたユーザーの情報やサービス利用履歴を確認できる画面です。

認証されたユーザー情報を基に、ユーザーごとのデータを取得するように実装しました。
...
일반 사용자와 관리자 권한을 분리하고 관리자만 접근할 수 있는 기능을 제공하는 화면입니다.

로그인한 사용자의 `role` 정보를 기준으로 일반 사용자 화면과 관리자 화면을 구분하여 처리했습니다.
...
이를 통해 로그인 여부뿐만 아니라 사용자의 권한에 따른 접근 제어와 UI 분리를 구현했습니다.

### 🇯🇵

一般ユーザーと管理者の権限を分け、管理者のみ利用できる機能を提供する画面です。

ログインユーザーの `role` 情報を基に、一般ユーザー用画面と管理者用画面を分けるように実装しました。

これにより、ログイン状態だけではなく、ユーザー権限に応じたアクセス制御とUIの分離を実装しました。
...
| Main                          | Search                            |
| ----------------------------- | --------------------------------- |
| ![Main](docs/images/main.png) | ![Search](docs/images/search.png) |

| Detail                            | Book Request                        |
| --------------------------------- | ----------------------------------- |
| ![Detail](docs/images/detail.png) | ![Request](docs/images/request.png) |

| My Page                           | Admin                           |
| --------------------------------- | ------------------------------- |
| ![MyPage](docs/images/mypage.png) | ![Admin](docs/images/admin.png) |
